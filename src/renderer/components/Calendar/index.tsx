...
interface CalendarProps {
  showCalendarSelections?: boolean;
  children?: React.ReactNode;
  compact?: boolean;
}

export default function Calendar({ showCalendarSelections = true, children, compact = false }: CalendarProps) {
  const intl = useIntl();
  ...
  // Fetch calendars when connected and selections are shown
  useEffect(() => {
    if (showCalendarSelections && googleCalendarConnected && googleCalendarToken && googleCalendarStore.calendars.length === 0) {
      googleCalendarStore.fetchCalendars();
    }
  }, [showCalendarSelections, googleCalendarConnected, googleCalendarToken, googleCalendarStore.calendars.length, googleCalendarStore.fetchCalendars]);

  useEffect(() => {
    if (showCalendarSelections && microsoftCalendarConnected && microsoftCalendarToken && microsoftCalendarStore.calendars.length === 0) {
      microsoftCalendarStore.fetchCalendars();
    }
  }, [showCalendarSelections, microsoftCalendarConnected, microsoftCalendarToken, microsoftCalendarStore.calendars.length, microsoftCalendarStore.fetchCalendars]);
  ...
  return (
    <Box sx={{ p: compact ? 0.5 : 1, width: '100%', maxWidth: 280, margin: 'auto' }}>
      {/* Calendar Header */}
      ...
      {/* Calendar Selections - Only show if enabled */}
      ...
      {/* Custom children content (for scheduled tasks, etc.) */}
      {children && (
        <>
          {showCalendarSelections && <Divider sx={{ my: 2 }} />}
          {children}
        </>
      )}
    </Box>
